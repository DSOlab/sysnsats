...
int main(int argc, char *argv[]) {
  ...
  /* Test 1: Testing attitude stream with normal dates. No errors */
  {
    try {
      ...
      att = new MeasuredAttitude(SATELLITE::JASON1, argv[1]);
    ...
    auto t = MjdEpoch(52261, FractionalSeconds(86393.123));
  ...
  /* Test 2: Go (way) backwards in time; should fail */
  {
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(SATELLITE::JASON1, argv[1],
                                 MjdEpoch(52261, FractionalSeconds(86399.123)));
    ...
    auto t = MjdEpoch(52261, FractionalSeconds(0.123));
  ...
  /* Test 3: Go (way) backwards in time; should fail (same as above). */
  {
    ...
    auto t = MjdEpoch(52261, FractionalSeconds(1043.123));
    if (att->attitude_at(t, data)) {
      fprintf(stderr, "ERROR Failed getting measured attitude! (tc 32)\n");
    }

    /* search some for nine days beofore; should not be in stream! */
    t = MjdEpoch(52260, FractionalSeconds(13.123));
    if (att->attitude_at(t, data) != 30) {
  ...
  {
    /* the last record included in the file: 52267 7.220546986302e+03 */
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(
          SATELLITE::JASON1, argv[1],
          MjdEpoch(52267, FractionalSeconds(7.220546986302e+03 - 200e0)));
    ...
    auto t = MjdEpoch(52267, FractionalSeconds(7.220546986302e+03 + 2e0));
