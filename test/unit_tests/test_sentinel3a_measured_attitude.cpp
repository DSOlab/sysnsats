...
int main(int argc, char *argv[]) {
  ...
  /* Test 1: Testing attitude stream with normal dates. No errors */
  {
    try {
      ...
      att = new MeasuredAttitude(SATELLITE::SENTINEL3A, argv[1]);
    ...
    auto t = MjdEpoch(58472, FractionalSeconds(86399.123));
  ...
  /* Test 2: Go (way) backwards in time; should fail */
  {
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(SATELLITE::SENTINEL3A, argv[1],
                                 MjdEpoch(58472, FractionalSeconds(86113.123)));
    ...
    auto t = MjdEpoch(58472, FractionalSeconds(13.123));
  ...
  {
    /* the last record included in the file: 58475 4.617024029139e+01 */
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(
          SATELLITE::SENTINEL3A, argv[1],
          MjdEpoch(58475, FractionalSeconds(4.617024029139e+01 - 200e0)));
    ...
    auto t = MjdEpoch(58475, FractionalSeconds(4.617024029139e+01 + 2e0));
