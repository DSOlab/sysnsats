...
int main(int argc, char *argv[]) {
  ...
  /* Test 1: Testing attitude stream with normal dates. No errors */
  {
    try {
      ...
      att = new MeasuredAttitude(SATELLITE::SENTINEL6A, argv[1]);
    ...
    auto t = MjdEpoch(60659, FractionalSeconds(86399.123));
  ...
  /* Test 2: Go (way) backwards in time; should fail */
  {
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(SATELLITE::SENTINEL6A, argv[1],
                                 MjdEpoch(60660, FractionalSeconds(86113.123)));
    ...
    /* search attitude for some days before; should not be in stream! */
    auto t = MjdEpoch(60660, FractionalSeconds(13.123));
  ...
  /* Test 3: Go (way) backwards in time; should fail (same as above). */
  {
    ...
    auto t = MjdEpoch(60666, FractionalSeconds(86399.123));
    if (att->attitude_at(t, data)) {
      fprintf(stderr, "ERROR Failed getting measured attitude! (tc 32)\n");
    }

    /* search some for days beofore; should not be in stream! */
    t = MjdEpoch(60666, FractionalSeconds(13.123));
    if (att->attitude_at(t, data) != 30) {
  ...
  {
    /* the last record included in the file: 60667 4.617024029139e+01 */
    try {
      /* get the measured attitude stream for the satellite */
      att = new MeasuredAttitude(
          SATELLITE::SENTINEL6A, argv[1],
          MjdEpoch(60667, FractionalSeconds(4.617024029139e+01 - 200e0)));
    ...
    auto t = MjdEpoch(60667, FractionalSeconds(4.617024029139e+01 + 2e0));
